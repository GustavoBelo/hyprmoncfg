...
func TestApplyBestRefusesUnmanagedMonitorConfig(t *testing.T) {
	env := newApplyBestTestEnv(t, `[{"id":1,"name":"eDP-1","description":"Framework Panel","make":"Framework","model":"Panel","serial":"A1","width":2880,"height":1800,"refreshRate":120,"x":0,"y":0,"scale":1.5,"transform":0,"disabled":false,"mirrorOf":""}]`)
	original := []byte("# Hand-written monitor rules\nmonitor = eDP-1,preferred,auto,1\n")
	if err := os.WriteFile(env.monitorsConfPath, original, 0o644); err != nil {
		t.Fatalf("write unmanaged monitors.conf: %v", err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "was not generated by hyprmoncfg") {
		t.Fatalf("expected daemon to refuse unmanaged monitor config, got %v", err)
	}

	got, err := os.ReadFile(env.monitorsConfPath)
	if err != nil {
		t.Fatalf("read unmanaged monitors.conf: %v", err)
	}
	if string(got) != string(original) {
		t.Fatalf("daemon changed unmanaged monitor config:\n%s", got)
	}
}
