...
type Engine struct {
	...
	AllowUnmanagedOverwrite bool
	Logf                    func(format string, args ...any)
}

type UnmanagedMonitorConfigError struct {
	Path            string
	AlternativePath string
}

func (e *UnmanagedMonitorConfigError) Error() string {
	return fmt.Sprintf(
		"%s already exists and was not generated by hyprmoncfg; refusing to replace it (use --monitors-conf %s and include that file, or approve the overwrite interactively)",
		e.Path,
		e.AlternativePath,
	)
}
...
func (e Engine) Apply(ctx context.Context, p profile.Profile, monitors []hypr.Monitor, modearg ...applyMode) (RevertState, error) {
	...
	if backup.Exists && !config.IsGeneratedMonitorsConfig(backup.Content) && !e.AllowUnmanagedOverwrite {
		return RevertState{}, &UnmanagedMonitorConfigError{
			Path:            resolvedConfig.MonitorsPath,
			AlternativePath: alternativeMonitorsPath(resolvedConfig.MonitorsPath),
		}
	}
...
func alternativeMonitorsPath(path string) string {
	...
	return filepath.Join(filepath.Dir(path), "hyprmoncfg-monitors"+ext)
}
